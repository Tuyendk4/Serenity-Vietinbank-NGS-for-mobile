...
import static vn.vietinbank.api.common.ParamsHelper.createParams;
...
public class CommonSteps {
    ...
    public void so_sanh_response_thuc_te_voi_mong_muon() throws JSONException {
    ...
    public void so_sanh_response_thuc_te_voi_mong_muon_va_khong_so_sanh_cac_truong(String ignoreFields) throws JSONException {
        JSONObject actual = Serenity.sessionVariableCalled(RESPONSE);
        JSONObject expected = Serenity.sessionVariableCalled(RESPONSE_EXPECTED);
        applyIgnoreFields(actual, expected, ignoreFields);
        JSONAssert.assertEquals(actual, expected, true);
    }

    @And("so sánh các trường có jsonPath {string} của response thực tế với mong muốn")
    public void so_sanh_cac_truong_co_jsonpath_cua_response_thuc_te_voi_mong_muon(String jsonPath) throws JSONException {
        JSONObject actual = Serenity.sessionVariableCalled(RESPONSE);
        JSONObject expected = Serenity.sessionVariableCalled(RESPONSE_EXPECTED);
        Object[] result = compareJSONObjectsWithJsonPath(actual, expected, jsonPath);
        String actualString = (String) result[0];
        String reason = (String) result[1];
        Assert.assertEquals(reason, TRUE, actualString);
    }

    @And("lấy data with json path {string} and save to serenity variable {string}")
    public void lay_data_with_json_path_and_save_to_serenity_variable(String jsonPath, String serenityVariable) throws JSONException {
        JSONObject actual = Serenity.sessionVariableCalled(RESPONSE);
