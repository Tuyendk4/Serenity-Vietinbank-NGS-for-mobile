...
public class BaseScreen {
    ...
    Logger logger = LogManager.getLogger(BaseScreen.class);
    ...
    public void clearText(WebElement we) {
        try {
            logger.info("Clear text on mobile element located by {}", we);
            we.clear();
            logger.info("Cleared text mobile element located by {} successfully", we);
        } catch (Exception e) {
            logger.error("Cannot clear text mobile element located by ''{}''. Root cause: {}",
                we, e.getMessage());
            ReportPortal.emitLog("Cannot click clear text mobile element", LogLevel.ERROR.name(),
    ...
    public boolean waitForElementInvisible(WebElement we, int timeOut) {
        try {
            logger.info("Waiting for mobile element {} to be invisible", we);
            WebDriverWait wait = new WebDriverWait(appiumDriver, Duration.ofSeconds(timeOut));
            Boolean invisibleStatus = wait.until(ExpectedConditions.invisibilityOf(we));
            if (invisibleStatus) {
                logger.info("Mobile element located by {} is invisible", we);
                return true;
            }
        } catch (Exception e) {
            logger.error(
    ...
    public void scrollUpElement(WebElement elementParent) {
        ...
        swipe.addAction(finger.createPointerMove(Duration.ofMillis(1000), Origin.viewport(), end.getX(), end.getY()));
